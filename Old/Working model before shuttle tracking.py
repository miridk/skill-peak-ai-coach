...
# ✅ Acceleration overlay settings (NEW)
ACCEL_SMOOTH_ALPHA = 0.65   # EMA smoothing for acceleration (0..1)
ACCEL_CAP_MPS2 = 11.0        # cap (m/s^2). Tune 5-9 for badminton
ACCEL_DEADZONE = 0.10       # m/s^2 deadzone around 0 (reduces flicker)
...
# Panel layout
PANEL_WIDTH = 160   # fast bredde i pixels (typisk 140–180)
PANEL_HEIGHT = 62
PANEL_ALPHA = 0.28

# ✅ Ring position + open arc settings
RING_Y_OFFSET_PX = 18         # flyt ringen op under spilleren (tune 15-20)
RING_ARC_SAMPLES = 64         # glathed på buen
...
# ✅ NEW: meters -> px (for drawing guide-lines)
...
# ✅ NEW: draw net + short service lines + centerline
def draw_court_guides(frame: np.ndarray, H: np.ndarray) -> None:
    """
    Tegner guide-linjer på videoen baseret på badmintonmål (meter) -> pixel via homography.
    Linjer:
      - Net (y = L/2)
      - Short service lines (±1.98m fra net)
      - Centerline (x = W/2)
    """
    mid_y = COURT_L / 2.0
    short = 1.98  # afstand fra net til short service line (singles/doubles)
...
# ✅ UPDATED: Zone-definition = side of net + front/back on that side + left/right
def classify_zone(x_m: float, y_m: float) -> str:
    """
    Zone-definition der matcher badmintonlogik:
      - FAR/NEAR: hvilken side af nettet du er på (y < mid = FAR)
      - FRONT/BACK: hvor tæt på nettet du er på DIN side (<= 1.98m = FRONT)
      - LEFT/RIGHT: venstre/højre halvdel (x)
    """
    mid = COURT_L / 2.0
    side_group = "FAR" if y_m < mid else "NEAR"

    dist_to_net = abs(y_m - mid)          # afstand til net
...
def draw_player_overlay(
    ...
    ring_color: Tuple[int, int, int] = RING_COLOR,   # ✅ NEW
    ...
    # ---------- Foot ring (OPEN ARC) + subtle shadow ----------
    rx = int(w * 0.38)
    ry = int(w * 0.18)
    ...
    draw_open_ellipse_arc(
        ...
        start_rad=arc_start,
        end_rad=arc_end,
        color=ring_color,           # ✅ dynamic color
...
def update_live_stats(states: Dict[int, LiveState], stable_id: int, t: float, x_m: float, y_m: float) -> Tuple[float, float, float]:
    """
    Returns:
      speed_kmh_ema, total_dist_m, accel_mps2_ema (signed)
    """
    if stable_id <= 0:
        return 0.0, 0.0, 0.0
    ...
    if step <= DIST_JUMP_CAP_M:
        ...
        # Signed acceleration from smoothed speed
...
def main():
    ...
    try:
        while True:
            ...
            if detections.class_id is not None and len(detections) > 0:
                ...
                if detections.tracker_id is not None and len(detections) > 0:
                    ...
                    for r, xyxy in zip(det_rows, detections.xyxy):
                        ...
                        sp_kmh, dist_m, accel_mps2 = update_live_stats(live_states, sid, timestamp_s, r["x_m"], r["y_m"])
            ...
            # ✅ NEW: draw guide-lines (net + short lines + centerline)
            try:
