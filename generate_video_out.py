# Hybrid upgrade for your badminton tracker
# Focus: stronger identity association via appearance + pose signature + court constraints
# Note: This version does NOT include full SAM2 inference. Instead it adds a
# 'rescue mode' hook for ambiguous / overlap frames so you can integrate SAM2 later
...
def point_in_court_asymmetric_margin(
    ...
    is_above_top = y < proj_y
    return bool(is_above_top and perp_dist <= top_margin)
...
def depth_role_from_y(y_m: float) -> str:
    mid = COURT_L / 2.0
    dist_to_net = abs(y_m - mid)
    return "FRONT" if dist_to_net <= 1.98 else "BACK"
...
def extract_pose_signature(pose_res) -> Optional[np.ndarray]:
    ...
    shoulder_mid = np.array([(ls[0] + rs[0]) * 0.5, (ls[1] + rs[1]) * 0.5], dtype=np.float32)
    hip_mid = np.array([(lh[0] + rh[0]) * 0.5, (lh[1] + rh[1]) * 0.5], dtype=np.float32)
    ankle_mid = np.array([(la[0] + ra[0]) * 0.5, (la[1] + ra[1]) * 0.5], dtype=np.float32)
...
class IdentityManager:
    ...
    def _pose_cost(self, tr: IdentityTrack, det: dict, ambiguous: bool) -> float:
        ...
        sim_ema = 0.0
        if tr.pose_feat is not None and feat is not None:
            sim_ema = clamp01((cosine_sim(tr.pose_feat, feat) + 1.0) * 0.5)
        if feat is None:
    ...
    def _total_cost(self, tr: IdentityTrack, det: dict, frame_idx: int, occluded: bool, ambiguous: bool) -> float:
        return (
            ...
            + W_POSE * self._pose_cost(tr, det, ambiguous)
            + self._role_cost(tr, det)
        )
    ...
    def _apply_teammate_constraints(self, proposed: Dict[int, int], detections: List[dict]) -> Dict[int, int]:
        ...
        for a, b in teams:
            ...
            if sep >= TEAMMATE_MIN_SEP_M:
                continue
            # keep current mapping; the penalty is effectively handled by cost thresholds
            out[a] = ia
            out[b] = ib
        return out
    ...
    def _update_track(self, sid: int, det: dict, frame_idx: int, occluded: bool, ambiguous: bool) -> None:
        ...
        if new_pose is not None:
            if tr.pose_feat is None:
                ...
                if n > 1e-8:
                    tr.pose_feat = (tr.pose_feat / n).astype(np.float32)
            freeze_bank = (occluded and OCCLUSION_FREEZE_BANK_UPDATE) or (ambiguous and AMBIGUOUS_FREEZE_BANK_UPDATE)
            if not freeze_bank:
...
def maybe_rescue_assignment(det_rows: List[dict], ambiguous_flags: List[bool], occlusion_flags: List[bool]) -> None:
    """
    Placeholder for SAM2 or mask-based rescue.
    Current behavior: mark frames where you should later run SAM2 and keep identity banks frozen.
    """
    ...
    try:
        while True:
            ...
            if detections.class_id is not None and len(detections) > 0:
                ...
                if detections.tracker_id is not None and len(detections) > 0:
                    ...
                    if not bootstrapped:
                        ...
                        # get flags once to allow rescue-hook markers
            ...
            try:
                ...
                pass
            cv2.putText(annotated, "Hybrid identity: CLIP + pose signature + court constraints + rescue hook", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.72, (255, 255, 255), 2, cv2.LINE_AA)
            cv2.putText(annotated, f"conf={CONF} iou={IOU} imgsz={IMGSZ}", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2, cv2.LINE_AA)
